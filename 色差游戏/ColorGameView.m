...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    CGRect rect = CGRectMake(0, (KH-KW)/2, KW, KW);
    ...
    if (self != nil) {
        ...
        _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, KW, KW) collectionViewLayout:_layout];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == _happenNum) {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"count" object:[NSString stringWithFormat:@"%ld", (long)_count]];
        [_collectionView reloadData];
    } else {
        //点错了就终止游戏
        NSLog(@"game over");
        [SVProgressHUD setDefaultStyle:SVProgressHUDStyleDark];
        [SVProgressHUD showInfoWithStatus:@"This one is not!!!"];
    }
